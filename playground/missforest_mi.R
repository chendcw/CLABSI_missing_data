## this is for imputation method: missforest + mi ##
...
for (f in train_files){
  ...
  # keep outcome & missing indicators separately
  data_train_outcome <- data_train %>% 
    select(eventtime, type, functioneelDossierNr, CAT_catheter_episode, LM, ends_with("_missing"))
  
  data_test_outcome <- data_test %>% 
    select(eventtime, type, functioneelDossierNr, CAT_catheter_episode, LM, ends_with("_missing"))
  
  data_train <- data_train %>% 
    select(-c(eventtime, type, ends_with("_missing")))
  ...
  data_train_bin <- create_new_variable(data_train_bin) %>% filter(LM<=30) %>% select(predictors_set, vars_id_outcome, c("id", "Tstart"))
  data_test_bin <- create_new_variable(data_test_bin) %>% filter(LM<=30) %>% select(predictors_set, vars_id_outcome, c("id", "Tstart"))
