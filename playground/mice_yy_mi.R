## this is for imputation method: mice_yy + mi ##
...
for (f in train_files){
  ...
  for (i in 1:10) {
    ...
    complete_data <- create_new_mice3(complete(MI, action = i))  # Extract complete dataset
  ...
  for (i in 1:10) {
    ...
    complete_data <- create_new_mice3(complete(imp.test, action = i))  # Extract complete dataset
  ...
  for (i in 1:length(imp_train_list)) {
    preds <- predictLM_CSC(model_info_list[[i]], predictors_set, imp_test_list[[i]], horizon = 7, primary_event = 1)
    preds_list[[i]] <- preds
  }
