## this is for imputation method: locf + mi ##
...
for (f in train_files){
  ...
  vars_impute_median <- setdiff(vars_impute_median,c("CAT_lumens_Dialysis_CVC","functioneelDossierNr","CAT_catheter_episode","eventtime","CAT_lumens_Total"))
  vars_impute_mode <- setdiff(vars_impute_mode, c("CAT_lumens_Port_a_cath","type", vars_impute_mode[grepl("_missing$", vars_impute_mode)]))
  ...
  # start locf imputation for train data (baseline replaced with data_train_base_imputed)
  ...
  data_train_imputed <- data_train %>%
    ...
    arrange(LM) %>%
    fill(-c(LM, type, eventtime), .direction = "down") %>%
    # mutate(across(-c("LM", "type", "eventtime"), ~na.locf(.))) %>%
    ungroup() 
  
  # start locf imputation for test data (baseline replaced with data_test_base_imputed)
  
  data_test[data_test$LM == 0, ] <- data_test_base_imputed
  
  data_test_imputed <- data_test %>%
    filter(LM <= 30) %>%
    group_by(functioneelDossierNr, CAT_catheter_episode) %>%
    arrange(LM) %>%
    fill(-c(LM, type, eventtime), .direction = "down") %>%
  ...
  data_train_bin <- create_new_variable(data_train_bin) %>% select(predictors_col, 
                                                                   vars_id_outcome, 
                                                                   c("id", "Tstart"),
  ...
  data_test_bin <- create_new_variable(data_test_bin) %>% select(predictors_col, 
                                                                 vars_id_outcome, 
                                                                 c("id", "Tstart"),
                                                                 c("ADM_admission_source_binary_all_Home_missing",
                                                                   "CARE_NEU_GCS_score_last_missing",
